...
RSpec.describe Product, type: :model do
  ...
  context 'when product has invalid attributes' do
    it "is not valid without a title" do
      expect(build(:product, title: '')).to_not be_valid
    end

    it "is not valid when has invalid price" do
      expect(build(:product, price: 5)).to_not be_valid
    end

    it "is not valid without code" do
      expect(build(:product, code: '')).to_not be_valid
    end

    it "is not valid without bar code" do
      expect(build(:product, bar_code: '')).to_not be_valid
    end

    it "is not valid without manufacturer" do
      expect(build(:product, manufacturer: '')).to_not be_valid
    end

    it "is not valid without model" do
      expect(build(:product, model: '')).to_not be_valid
    end

    it "is not valid when has invalid bar code" do
      expect(build(:product, bar_code: 123456789)).to_not be_valid
    end

    it 'is not valid when has not a category' do
      expect(build(:product, category: nil)).to_not be_valid
    end
  end
end
